...
/**
 * Folders never collected, checked as a direct child of the given directory kind's own root (not
 * matched anywhere deeper - a folder is only "special" in the specific place RRF actually puts it).
 * `M956`'s default accelerometer-sample output is `0:/sys/accelerometer`
 * (Duet3D/wiki-content, Reference/Gcodes.md, "M956: Collect accelerometer data and write to file") -
 * resonance-tuning capture data regenerated by the user's own input-shaping workflow, not
 * configuration a restore should bring back, and potentially large (many samples across 3 axes).
 */
export const EXCLUDED_FOLDERS: Partial<Record<BackupDirKind, ReadonlySet<string>>> = {
	system: new Set(["accelerometer"]),
};
